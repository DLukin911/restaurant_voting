...
class AdminAccountControllerTest extends AbstractControllerTest {
    ...
    private static final String REST_URL = AdminAccountController.REST_URL + '/';
    ...
    void getAll() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(USER_MATCHER.contentJson(List.of(admin, user, user2, user3, newUserForTestApi, newUserForVote)));
    }
}